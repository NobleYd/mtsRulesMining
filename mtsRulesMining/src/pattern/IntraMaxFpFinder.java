 ...
 * Return the intra max frequent pattern (substring) set, and corresponding position list.<br/>
 * 
 * @author Yi Zhao
 */
public class IntraMaxFpFinder {
	...
	public IntraMaxFpFinder(DiscretizedData discretizedData, int minSupportCount, Integer windowSize) {
		super();
	...
	public Map<IntraPattern, IntraPattern> run() {
		List<IntraPattern> noMaxIntraFps = new ArrayList<IntraPattern>();
				...
				for (IntraPattern intraFp2 : intraFpsI_1.values()) {
					if (intraFp2.getPattern().startsWith(intraFp1Suffix)) {
						...
						if (generatedpositions.size() >= minSupportCount) {
							...
							noMaxIntraFps.add(intraFp1);
							noMaxIntraFps.add(intraFp2);
		...
		for (IntraPattern intraFp : noMaxIntraFps) {
