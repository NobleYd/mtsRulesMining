 ...
 * Return the intra closed frequent pattern (substring) set, and corresponding position list.<br/>
 * 
 * @author Yi Zhao
 */
public class IntraClosedFpFinder {
	...
	public IntraClosedFpFinder(DiscretizedData discretizedData, int minSupportCount, Integer windowSize) {
		super();
	...
	public Map<IntraPattern, IntraPattern> run() {
		List<IntraPattern> noClosedIntraFps = new ArrayList<IntraPattern>();
				...
				for (IntraPattern intraFp2 : intraFpsI_1.values()) {
					if (intraFp2.getPattern().startsWith(intraFp1Suffix)) {
						...
						if (generatedpositions.size() >= minSupportCount) {
							...
							if (generatedpositions.size() == intraFp1.getPositions().size()) {
								noClosedIntraFps.add(intraFp1);
							}
							if (generatedpositions.size() == intraFp2.getPositions().size()) {
								noClosedIntraFps.add(intraFp2);
							}
		...
		for (IntraPattern intraFp : noClosedIntraFps) {
			intraFps.remove(intraFp);
		}
